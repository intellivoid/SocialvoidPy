...
class Account:
    ...
    async def set_profile_picture(self, document: typing.Union[str, types.Document]) -> bool:
        if isinstance(document, types.Document):
            document = document.id
        return (await self._sv.make_request(Request('account.set_profile_picture', {'session_identification': create_session_id(self._sv.session), 'document': document}))).unwrap()
